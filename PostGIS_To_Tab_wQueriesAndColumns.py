...
class TabExportUtility :
    def __init__(self, input_databases, input_tables, input_server, output_path):
        # create databases list and table list
        self.databases = input_databases
        print "Databases:", databases
        self.tables = input_tables
        print "Table list:", tableList
        self.server = input_server
        print "Server:", tableList
        self.outputBase = output_path
        print "Table list:", output_path
    def connect_database(self, target_database, username, password, port = "5423"):
        uri = QgsDataSourceURI()
        uri.setConnection(self.server, port, target_database, username, password)
        return uri
    def set_encoding(self, new_encoding = "utf-8"):
        self.encoding = new_encoding
    def set_coordsys(self, input_coordsys = QgsCoordinateReferenceSystem(3395, QgsCoordinateReferenceSystem.EpsgCrsId)):
        self.coordsys = input_coordsys
    def export(self, input_username, input_password):
        for database in self.databases:
            current_uri = self.connect_database(database, input_username, input_password)
            print "Connection to", self.server, database, "established."
            # go through every table in the list and turn it into a vector layer
            for table in self.tables:
                print "Conection to", database, ": ", table, " established."
                current_uri.setDataSource("public", table, "way")
                ...
                if table == "planet_osm_line":
                    ...
                    output = self.outputBase + "/" + database + "/" + railway[0] + ".tab"

                    QgsVectorFileWriter.writeAsVectorFormat(mapLayer, output, self.encoding, self.coordsys, "MapInfo File", False)
                    #QgsVectorFileWriter.writeAsVectorFormat(vlayer, output, encoding, coordsys, "MapInfo File", True)
                    print "Translation of " + self.outputBase + database + "/" + railway[0] + ".tab successful."
...
my_util = TabExportUtility()
my_util.set_coordsys(QgsCoordinateReferenceSystem(3395, QgsCoordinateReferenceSystem.EpsgCrsId))
my_util.set_encoding("utf-8")
my_util.export("username", "password")
